...
# The dashboard imports these scripts rather than duplicating their calculations.
try:
...
# The dashboard must not depend on demo data at startup.
...
def normalise_live_replay_frame(stored_data):
    """Return uploaded/preprocessed data in the column format expected by the live replay section."""
    if not stored_data:
        return pd.DataFrame()

    frame = pd.DataFrame(stored_data).copy()
    if frame.empty:
        return frame
    ...
    if "time_s" not in frame.columns:
        for candidate in ["elapsed_time_s", "timestamp", "time", "t_s", "time_step", "timestep"]:
            if candidate in frame.columns:
                frame["time_s"] = frame[candidate]
                break

    if "time_s" in frame.columns:
        numeric_time = pd.to_numeric(frame["time_s"], errors="coerce")
        if numeric_time.notna().any():
            frame["time_s"] = numeric_time
        else:
            datetime_time = pd.to_datetime(frame["time_s"], errors="coerce")
            frame["time_s"] = (datetime_time - datetime_time.iloc[0]).dt.total_seconds()

    if "speed_kmh" not in frame.columns:
        if "vehicle_speed" in frame.columns:
            frame["speed_kmh"] = pd.to_numeric(frame["vehicle_speed"], errors="coerce")
    ...
    if "gear" not in frame.columns:
        speed = pd.to_numeric(frame.get("speed_kmh", 0), errors="coerce").fillna(0)
        frame["gear"] = np.select(
            [speed < 2, speed < 15, speed < 30, speed < 45, speed < 65, speed < 90],
            [0, 1, 2, 3, 4, 5],
            default=6,
        )
    frame["gear"] = pd.to_numeric(frame["gear"], errors="coerce").fillna(0).astype(int)

    required = ["time_s", "speed_kmh"]
    missing = [col for col in required if col not in frame.columns]
    if missing:
        raise ValueError(f"Processed data is missing required live replay columns: {missing}")

    frame = frame.replace([np.inf, -np.inf], np.nan).dropna(subset=["time_s", "speed_kmh"])
    frame = frame.sort_values("time_s").reset_index(drop=True)
    return frame


def downsample_live_replay_frame(frame, max_points=5000):
    """Keep browser graph rendering stable by drawing a representative subset once."""
    if frame is None or frame.empty or len(frame) <= max_points:
        return frame

    keep_indices = np.linspace(0, len(frame) - 1, max_points).astype(int)
...
def predict_traffic_from_processed_frame(frame, filename):
    ...
    if traffic_model is None:
        if joblib is None:
            unavailable = "joblib is not installed or could not be imported."
        else:
            unavailable = f"traffic_model.pkl was not found at {TRAFFIC_MODEL_PATH}. Put traffic_model.pkl in the same folder as this dashboard file."
        return {
            "available": False,
            "message": unavailable,
        }
...
app.layout = html.Div(
    [
        ...
        # This prevents the slider/playback callbacks from sending the full 38k+ row CSV
        # back to Python every 250 ms.
...
# Will's code for playback controls, timeline label, and main dashboard indicators and plots.
        ...
        html.Div(
            [
                ...
                html.Div([dcc.Graph(id="gear-ratio-plot")], style=card_style()),
                html.Div([dcc.Graph(id="fan-coolant-plot")], style=card_style()),
...
def process_uploaded_file(contents, filename):
    ...
    try:
        ...
        return (
            ...
            driver_analysis,
            traffic_analysis,
            f"Loaded {filename} with {len(processed_df)} rows. Gear, fan speed, traffic prediction, acceleration, total distance, Granite graph analysis, and driver behaviour analysis have been updated for this file.",
        )
...
def update_time_slider_limits(stored_data):
    if not stored_data:
        return 0, {0: "Upload CSV"}

    try:
        frame = normalise_live_replay_frame(stored_data)
    except Exception:
    ...
    return len(frame) - 1, slider_marks(frame)
...
def advance_timeline(_, __, playback_state, playback_rate, current_value, slider_max):
    ...
    max_index = int(slider_max or 0)
    if max_index <= 0:
        return 0

    next_value = int(current_value or 0) + int(playback_rate or 1)
...
def initialise_live_replay_graphs(stored_data):
    ...
    Do not use time-slider as an Input here. Redrawing five Plotly figures on
    every playback tick is what made the dashboard feel as if it was iterating
    painfully slowly. The slider/cards/gauges update during playback; the line
    graphs remain fixed and lightweight.
    """
...
def update_live_replay_cards_and_gauges(index, stored_data):
    """Server-side replacement for the old clientside callback.

    The previous JavaScript clientside callback caused the browser error:
    dc[namespace][function_name] is not a function. Keeping the callback in Python
    is slightly less fast, but it is much more robust across Dash versions.
    """
    if not stored_data:
        return (
            "Upload a CSV file to initialise the live replay dashboard.",
            "--", "",
            "--", "°C",
            "--", "%",
            "--", "%",
    ...
    try:
        ...
        message = f"Live replay failed: {e}"
        return (
            message,
            "--", "",
            "--", "°C",
    ...
    if frame.empty:
        return (
            "No valid live replay rows were found.",
            "--", "",
            "--", "°C",
            "--", "%",
            "--", "%",
    ...
    max_index = len(frame) - 1
    idx = max(0, min(int(index or 0), max_index))
    row = frame.iloc[idx]

    current_time = safe_float(row.get("time_s", 0.0))
    end_time = safe_float(frame.iloc[-1].get("time_s", current_time))

    speed = safe_float(row.get("speed_kmh", row.get("vehicle_speed", 0.0)))
    rpm = safe_float(row.get("engine_rpm", 0.0))

    gear = row.get("gear", np.nan)
    gear_value = "N" if pd.isna(gear) or int(round(float(gear))) == 0 else str(int(round(float(gear))))

    coolant = format_stat_value(row, "coolant_temp_c", ".1f")
    fan_pct = format_stat_value(row, "fan_speed_pct", ".0f")
    throttle = format_stat_value(row, "throttle_pct", ".1f")
    map_kpa = format_stat_value(row, "map_kpa", ".1f")
    maf = format_stat_value(row, "maf_gps", ".1f")

    return (
        f"Elapsed {format_time(current_time)} / {format_time(end_time)}",
        gear_value, "",
        coolant, "°C",
        fan_pct, "%",
        throttle, "%",
        map_kpa, "kPa",
        maf, "g/s",
        make_speedometer(speed),
        make_rpm_gauge(rpm),
        f"server-marker-at-{current_time}",
    )
...
def update_tts_status(read_clicks, stop_clicks, answer_text):
    """Server-safe placeholder for the old browser text-to-speech callback.

    This avoids the Dash clientside-callback registration error. Browser speech can be
    restored later by moving the JavaScript into an assets/*.js file.
    """
    trigger = callback_context.triggered[0]["prop_id"] if callback_context.triggered else ""

    if trigger == "tts-stop-button.n_clicks":
        return "Reading stopped."

    if trigger == "tts-read-button.n_clicks":
        if not answer_text:
            return "No Granite answer available to read."
        return "Text-to-speech is disabled in this server-safe build. The Granite answer is shown above."
