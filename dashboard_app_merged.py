            ...
            [
                ...
                dcc.Checklist(
                    ...
                    options=[
                        ...
                        {"label": "Granite graph analysis", "value": "granite"},
                        {"label": "Driver analysis/classification", "value": "driver"},
                    ],
                    value=["basic", "thermodynamics", "fuel", "braking", "granite", "driver"],
                    inline=True,
                    labelStyle={
                        "color": colors["text"],
                        "marginRight": "22px",
...
def toggle_graph_sections(selected_sections):
    ...
    granite_style = card_visible if "granite" in selected_sections else hidden
...
app.clientside_callback(
    ...
    function(index, storedData) {
        ...
        function getNumber(row, keys, fallback) {
        ...
        function getTime(row, firstRow) {
            const direct = getNumber(row, ["time_s", "elapsed_time_s", "timestamp", "time", "t_s", "time_step", "timestep"], NaN);
            if (Number.isFinite(direct)) {
                return direct;
            }

            const firstRaw = firstRow ? (firstRow.time_s ?? firstRow.elapsed_time_s ?? firstRow.timestamp ?? firstRow.time) : null;
            const raw = row ? (row.time_s ?? row.elapsed_time_s ?? row.timestamp ?? row.time) : null;
            const firstDate = Date.parse(firstRaw);
            const rowDate = Date.parse(raw);
            if (Number.isFinite(firstDate) && Number.isFinite(rowDate)) {
                return (rowDate - firstDate) / 1000.0;
            }
            return 0;
        }

        function stat(row, keys, digits) {
            const value = getNumber(row, keys, NaN);
            if (!Number.isFinite(value)) {
                return "--";
            }
        ...
        function makeSpeedometer(speedValue) {
            return {
                data: [{
                    type: "indicator",
                    mode: "gauge+number",
                    value: Number(speedValue || 0),
                    number: {suffix: " km/h", font: {size: 34}},
                    title: {text: "Vehicle speed"},
                    gauge: {
                        axis: {range: [0, 140], tickwidth: 1},
        ...
        function makeRpmGauge(rpmValue) {
            return {
                data: [{
                    type: "indicator",
                    mode: "gauge+number",
                    value: Number(rpmValue || 0),
                    number: {suffix: " rpm", font: {size: 30}},
                    title: {text: "Engine RPM"},
                    gauge: {
                        axis: {range: [0, 7000], tickwidth: 1},
        ...
        function moveMarker(graphId, xValue) {
            const host = document.getElementById(graphId);
            const plot = host ? host.querySelector(".js-plotly-plot") : null;
            if (plot && window.Plotly && plot.layout && plot.layout.shapes && plot.layout.shapes.length > 0) {
        ...
        const maxIndex = storedData.length - 1;
        const i = Math.max(0, Math.min(Number(index || 0), maxIndex));
        const row = storedData[i];
        const firstRow = storedData[0];
        const lastRow = storedData[maxIndex];

        const currentTime = getTime(row, firstRow);
        const endTime = getTime(lastRow, firstRow);
        const speed = getNumber(row, ["speed_kmh", "vehicle_speed"], getNumber(row, ["speed_ms"], 0) * 3.6);
        const rpm = getNumber(row, ["engine_rpm"], 0);

        let gear = getNumber(row, ["gear"], NaN);
        if (!Number.isFinite(gear)) {
            gear = speed < 2 ? 0 : speed < 15 ? 1 : speed < 30 ? 2 : speed < 45 ? 3 : speed < 65 ? 4 : speed < 90 ? 5 : 6;
        }
        ...
        return [
            ...
            gearDisplay,
            "",
            stat(row, ["coolant_temp_c"], 1),
            "°C",
            stat(row, ["fan_speed_pct"], 0),
            "%",
            stat(row, ["throttle_pct"], 1),
            "%",
            stat(row, ["map_kpa"], 1),
            "kPa",
            stat(row, ["maf_gps"], 1),
            "g/s",
            makeSpeedometer(speed),
            makeRpmGauge(rpm),
            "marker-at-" + currentTime
        ];
    }
    """,
